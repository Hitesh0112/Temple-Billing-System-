...
def _open_2():
    ...
    def save():
        ...
        df.to_csv('Nankodai.csv', mode='a', index=False, header=False)
...
def sample_bill():
    ...
    x = ['Koothan Sivanthi', 'Name', 'All Days', 'saturday only', 'sunday only', 'monday only',
         'saturday breakfast only', 'saturday lunch only', 'saturday dinner only',
         'saturday breakfast and lunch', 'saturday lunch and dinner',
         'sunday breakfast only', 'sunday lunch only', 'sunday dinner only', 'sunday breakfast and lunch',
         'sunday lunch and dinner', 'monday breakfast only', 'monday lunch only', 'total']
    translated = translator.translate(x, src='en', dest='ta')

    if name.get() != '':
        l1.insert(END, f"{translated[1].text}:\t{name.get()}\n")

    l1.insert(END, f"Name\tQty\tAmount\n")
    l1.insert(END, f"--------------------------------------------------\n")
    # ---------------------give 35 space for each line--------------------------

    if all_days_adults.get() != '':
        l1.insert(END, f"{translated[2].text}:\t\t\t{all_days_adults.get()}\t{all_days_adults_total.get()}\n")

    if saturday_only_adults.get() != '':
        l1.insert(END, f"{translated[3].text}:\t\t{saturday_only_adults.get()}\t{saturday_only_adults_total.get()}\n")

    if sunday_only_adults.get() != '':
        l1.insert(END, f"{translated[4].text}:\t{sunday_only_adults.get()}\t{sunday_lunch_only_adults_total.get()}\n")

    if monday_only_adults.get() != '':
        l1.insert(END, f"{translated[5].text}:\t{monday_only_adults.get()}\t{monday_lunch_only_adults_total.get()}\n")

    if saturday_breakfast_only_adults.get() != '':
        l1.insert(END,
                  f"{translated[6].text}:\t{saturday_breakfast_only_adults.get()}\t{saturday_breakfast_only_adults_total.get()}\n")

    if saturday_lunch_only_adults.get() != '':
        l1.insert(END,
                  f"{translated[7].text}:\t{saturday_lunch_only_adults.get()}\t{saturday_lunch_only_adults_total.get()}\n")

    if saturday_dinner_only_adults.get() != '':
        l1.insert(END,
                  f"{translated[8].text}:\t{saturday_dinner_only_adults.get()}\t{saturday_dinner_only_adults_total.get()}\n")

    if saturday_bf_and_lunch_adults.get() != '':
        l1.insert(END,
                  f"{translated[9].text}:\t{saturday_bf_and_lunch_adults.get()}\t{saturday_bf_and_lunch_adults_total.get()}\n")

    if saturday_lunch_and_dinner_adults.get() != '':
        l1.insert(END,
                  f"{translated[10].text}:\t{saturday_lunch_and_dinner_adults.get()}\t{saturday_lunch_and_dinner_adults_total.get()}\n")

    if sunday_breakfast_only_adults.get() != '':
        l1.insert(END,
                  f"{translated[11].text}:\t{sunday_breakfast_only_adults.get()}\t{sunday_breakfast_only_adults_total.get()}\n")

    if sunday_lunch_only_adults.get() != '':
        l1.insert(END,
                  f"{translated[12].text}:\t{sunday_lunch_only_adults.get()}\t{sunday_lunch_only_adults_total.get()}\n")

    if sunday_dinner_only_adults.get() != '':
        l1.insert(END,
                  f"{translated[13].text}:\t{sunday_dinner_only_adults.get()}\t{sunday_dinner_only_adults_total.get()}\n")

    if sunday_bf_and_lunch_adults.get() != '':
        l1.insert(END,
                  f"{translated[14].text}:\t{sunday_bf_and_lunch_adults.get()}\t{sunday_bf_and_lunch_adults_total.get()}\n")

    if sunday_lunch_and_dinner_adults.get() != '':
        l1.insert(END,
                  f"{translated[15].text}:\t{sunday_lunch_and_dinner_adults.get()}\t{sunday_lunch_and_dinner_adults_total.get()}\n")

    if monday_breakfast_only_adults.get() != '':
        l1.insert(END,
                  f"{translated[16].text}:\t{monday_breakfast_only_adults.get()}\t{monday_breakfast_only_adults_total.get()}\n")

    if monday_lunch_only_adults.get() != '':
        l1.insert(END,
                  f"{translated[17].text}:\t{monday_lunch_only_adults.get()}\t{monday_lunch_only_adults_total.get()}\n")
    # -----------------------------------------------------------------KIDS-----------------------------------------------------------------------------

    if all_days_kids.get() != '':
        l1.insert(END, f"{translated[2].text}:\t{all_days_kids.get()}\t{all_days_kids_total.get()}\n")

    if saturday_only_kids.get() != '':
        l1.insert(END, f"{translated[3].text}:\t{saturday_only_kids.get()}\t{saturday_only_kids_total.get()}\n")

    if sunday_only_kids.get() != '':
        l1.insert(END, f"{translated[4].text}:\t{sunday_only_kids.get()}\t{sunday_lunch_only_kids_total.get()}\n")

    if monday_only_kids.get() != '':
        l1.insert(END, f"{translated[5].text}:\t{monday_only_kids.get()}\t{monday_lunch_only_kids_total.get()}\n")

    if saturday_breakfast_only_kids.get() != '':
        l1.insert(END,
                  f"{translated[6].text}:\t{saturday_breakfast_only_kids.get()}\t{saturday_breakfast_only_kids_total.get()}\n")

    if saturday_lunch_only_kids.get() != '':
        l1.insert(END,
                  f"{translated[7].text}:\t{saturday_lunch_only_kids.get()}\t{saturday_lunch_only_kids_total.get()}\n")

    if saturday_dinner_only_kids.get() != '':
        l1.insert(END,
                  f"{translated[8].text}:\t{saturday_dinner_only_kids.get()}\t{saturday_dinner_only_kids_total.get()}\n")

    if saturday_bf_and_lunch_kids.get() != '':
        l1.insert(END,
                  f"{translated[9].text}:\t{saturday_bf_and_lunch_kids.get()}\t{saturday_bf_and_lunch_kids_total.get()}\n")

    if saturday_lunch_and_dinner_kids.get() != '':
        l1.insert(END,
                  f"{translated[10].text}:\t{saturday_lunch_and_dinner_kids.get()}\t{saturday_lunch_and_dinner_kids_total.get()}\n")

    if sunday_breakfast_only_kids.get() != '':
        l1.insert(END,
                  f"{translated[11].text}:\t{sunday_breakfast_only_kids.get()}\t{sunday_breakfast_only_kids_total.get()}\n")

    if sunday_lunch_only_kids.get() != '':
        l1.insert(END, f"{translated[12].text}:\t{sunday_lunch_only_kids.get()}\t{sunday_lunch_only_kids_total.get()}\n")

    if sunday_dinner_only_kids.get() != '':
        l1.insert(END, f"{translated[13].text}:\t{sunday_dinner_only_kids.get()}\t{sunday_dinner_only_kids_total.get()}\n")

    if sunday_bf_and_lunch_kids.get() != '':
        l1.insert(END,
                  f"{translated[14].text}:\t{sunday_bf_and_lunch_kids.get()}\t{sunday_bf_and_lunch_kids_total.get()}\n")

    if sunday_lunch_and_dinner_kids.get() != '':
        l1.insert(END,
                  f"{translated[15].text}:\t{sunday_lunch_and_dinner_kids.get()}\t{sunday_lunch_and_dinner_kids_total.get()}\n")

    if monday_breakfast_only_kids.get() != '':
        l1.insert(END,
                  f"{translated[16].text}:\t{monday_breakfast_only_kids.get()}\t{monday_breakfast_only_kids_total.get()}\n")

    if monday_lunch_only_kids.get() != '':
        l1.insert(END, f"{translated[17].text}:\t{monday_lunch_only_kids.get()}\t{monday_lunch_only_kids_total.get()}\n")
